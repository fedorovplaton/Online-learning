package _6_Collections.StreamApi.Mail;
...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailMessage extends AbstractSendable<String>{
        public MailMessage(String from, String to, String value) {
            super(from, to, value);
        }
    }
    ...
    public static class MailService <T> implements Consumer<AbstractSendable<T>> {
        Map box = new HashMap<String, List<T>>() {
            @Override
        ...
        public void accept(AbstractSendable<T> tAbstractSendable) {
            List temp = (List) box.get(tAbstractSendable.getTo()); // ???
            temp.add(tAbstractSendable.getContent());
        }
    }

    public interface Service <T>{
        void accept(Sendable sendable);
    }
    ...
    public static abstract class AbstractSendable <T> implements Sendable {
        ...
        public AbstractSendable(String from, String to, T value) {
            this.from = from;
            this.to = to;
            this.value = value;
        }
